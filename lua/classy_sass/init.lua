...
M.add_scss_comments = function()
    ...
    local lines = safe_operation(
        function()
            return vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
        end,
        "Failed to read buffer contents"
    )
    ...
    for line_num, line in ipairs(lines) do
        ...
        if not ok or result == false then
            notify_error(string.format("Error processing line %d: %s", line_num,
                ok and "Invalid selector" or result))
            return
        end
    end

    -- Safely update buffer contents
    local ok = safe_operation(
        function()
    ...
    -- Only write if configured to do so and update was successful
    if ok and M.opts.auto_write then
        safe_operation(
            function()
                vim.cmd("write")
            end,
            "Failed to write buffer to disk"
        )
    end
end
...
M.setup = function(user_opts)
    ...
    -- Safely set up keybinding
    if M.opts.keybinding then
        safe_operation(
            function()
                vim.keymap.set("n", M.opts.keybinding, ":AddSCSSComments<CR>", {
                    desc = "Add SCSS Comments",
                    silent = true,
                })
            end,
            "Failed to set up keybinding"
        )
    end
end
